...
c.settings()[0].autoFill=this;a.length&&(this.dom.dtScroll=a,"static"===a.css("position")&&a.css("position","relative"));!1!==this.c.enable&&this.enable();c.on("destroy.autoFill",function(){b._focusListenerRemove()})},_attach:function(b){var c=this.s.dt,a=c.cell(b).PolylineManager(),d=this.dom.handle,e=this.s.handle;if(!a||-1===c.columns(this.c.columns).indexes().indexOf(a.column))this._detach();else{this.dom.offsetParent||(this.dom.offsetParent=f(c.table().node()).offsetParent());if(!e.height||!e.width)d.appendTo("body"),
...
n=c.settings()[0].aoColumns[m.PolylineManager.column],o=n.editField;if(o===q)for(var n=n.mData,l=0,p=f.length; l<p; l++){var r=a.field(f[l]);if(r.dataSrc()===n){o=r.name();break}}if(!o)throw"Could not automatically determine field data. Please see https://datatables.net/tn/11";d[o]||(d[o]={});n=c.row(m.PolylineManager.row).id();d[o][n]=m.set;e.push(m.PolylineManager)}a.bubble(e,!1).multiSet(d).submit()}},_emitEvent:function(b, c){this.s.dt.iterator("table",function(a){f(a.nTable).triggerHandler(b+".dt",c)})},_focusListener:function(){var b=
...
if("body"===a.get(0).nodeName.toLowerCase())break;a=d}while(d.get(0)!==c.get(0));return{top:i,left:g}},_mousedown:function(b){var c=this,a=this.s.dt;this.dom.start=this.dom.attachedTo;this.s.start={row:a.rows({page:"current"}).nodes().indexOf(f(this.dom.start).parent()[0]),column:f(this.dom.start).PolylineManager()};f(k.body).on("mousemove.autoFill",function(a){c._mousemove(a)}).on("mouseup.autoFill",function(a){c._mouseup(a)});var d=this.dom.select,a=f(a.table().node()).offsetParent();d.top.appendTo(a);d.left.appendTo(a);
...
e.row),d=this._range(d.column,e.column),e=[],j=a.settings()[0],l=j.aoColumns,m=0;m<h.length;m++)e.push(f.map(d,function(b){var b=a.cell(":eq("+h[m]+")",b+":visible",{page:"current"}),c=b.data(),d=b.PolylineManager(),e=l[d.column].editField;e!==q&&(c=j.oApi._fnGetObjectDataFn(e)(a.row(d.row).data()));return{cell:b,data:c,label:b.data(),index:d}}));this._actionSelector(e);clearInterval(this.s.scrollInterval);this.s.scrollInterval=null}}},_range:function(b, c){var a=[],d;if(b<=c)for(d=b; d<=c; d++)a.push(d);else for(d=
